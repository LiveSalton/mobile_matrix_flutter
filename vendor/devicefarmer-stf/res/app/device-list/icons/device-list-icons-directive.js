...
, $rootScope
  ...
  function DeviceItem() {
    return {
      build: function() {
        ...
        selector.className = 'mm-device-select'
        selector.setAttribute('aria-label', '选择设备')
        var selectorIcon = document.createElement('i')
        selectorIcon.className = 'fa fa-check'
  ...
  , link: function(scope, element) {
        ...
        if (id) {
          ...
          if (scope.selectionMode) {
            scope.$applyAsync(function() {
              if (scope.selectedSerials[device.serial]) {
                delete scope.selectedSerials[device.serial]
              }
              else {
                scope.selectedSerials[device.serial] = true
              }
      ...
      scope.$watch('selectionMode', refreshSelectionState)
      scope.$watchCollection('selectedSerials', refreshSelectionState)
      scope.$watchCollection('screenshots', refreshSelectionState)
      ...
      function createItem(device) {
        ...
        item.id = id
        builder.update(item, device, scope.selectionMode, scope.selectedSerials, scope.screenshots)
        mapping[id] = device
