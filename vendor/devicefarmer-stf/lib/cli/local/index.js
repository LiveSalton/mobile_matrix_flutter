...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    .option('trusted-local', {
      describe: 'Bypass the login page with one fixed local STF identity.'
    , type: 'boolean'
    , default: false
    })
    .option('local-user-name', {
      describe: 'Display name for the trusted local STF identity.'
    , type: 'string'
    , default: 'administrator'
    })
    .option('local-user-email', {
      describe: 'Email key for the trusted local STF identity.'
    , type: 'string'
    , default: 'administrator@fakedomain.com'
    })
...
module.exports.handler = function(argv) {
  ...
  function run() {
    var authProc = argv.trustedLocal ? null : procutil.fork(
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--storage-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.port)
      ...
      // auth (not required for the explicit trusted-local profile)
    , authProc
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--websocket-url', argv.websocketUrl || util.format(
          ...
          if (argv.trustedLocal) {
            extra.push(
              '--trusted-local'
            , '--local-user-name', argv.localUserName
            , '--local-user-email', argv.localUserEmail
            )
          }
          return extra
        })()))
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
      ...
      , '--storage-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.storagePort)
        ])
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        , '--port', argv.port
        , '--app-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.appPort)
        , '--auth-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.authPort)
        , '--api-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.apiPort)
        , '--websocket-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.websocketPort)
        , '--storage-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.storagePort)
        , '--storage-plugin-image-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.storagePluginImagePort)
        , '--storage-plugin-apk-url'
        , util.format('http://%s:%d/', argv.publicIp, argv.storagePluginApkPort)
        ])
    ].filter(Boolean)
