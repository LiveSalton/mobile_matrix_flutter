...
Web 设备工具分为 Dashboard、Logs、Screenshots、Automation、File Explorer、Advanced、Info 七组；Flutter 工具箱展示 Dashboard、Logs、Automation、File Explorer、Advanced、Info 六组，截图复制收敛到设备信息栏快捷图标。Flutter 以 ADB 为桌面传输边界；已有 STF Socket.IO 屏幕流、触控和文本控制不迁移到工具服务。
...
- `DeviceWorkspace`：维护当前工具、输入控制器和加载状态；左侧为六组工具导航，右侧只渲染当前工具，所有内容放入可滚动区域。
- 页面双栏布局：左栏只承载手机实时画面和手机底部导航，并按当前手机屏幕宽高比计算左栏宽度，使实时画面贴合左栏边界；右栏顶部完整显示 AppHeader（品牌、设备选择、刷新设备和主题控制），下面再显示设备型号、分辨率、唯一 FPS，以及相邻的横屏、Wi‑Fi、蓝牙、显隐和复制截屏图标与设备工具箱，避免设备控制与手机画面争用垂直空间。横屏切换必须同步设备状态、流视口和渲染方向，并保持画面比例。
...
- 设备信息栏：左侧按设备型号、分辨率、FPS 排列；右侧将横屏、Wi‑Fi、蓝牙、屏幕显隐和复制截屏放入同一组，保证相关操作相邻且不跳动。
...
- 中文界面：Dashboard 卡片标题、说明和可见操作文案统一使用中文；高级剪贴板中心的输入框与按钮组使用同一条左侧基线，英文仅保留 URL、DeepLink、ADB、Shell 等必要技术名词及快捷控制悬停提示。页面文案统一通过 `L10n.of(context)` 读取，中文与英文资源分别维护在 `lib/l10n/intl_zh.arb` 和 `lib/l10n/intl_en.arb`，由 `intl_utils` 生成访问代码。
- ADB 远程网络调试：Dashboard 使用一个 `_toolScroll` 响应式 item 承载双列独立流；宽度达到与 Dashboard 栅格一致的 `640px` 时，左列依次排列快捷控制、URL 启动器、智能输入和应用管理，右列依次排列实时监控、剪贴板、Shell 和远程调试，每列各占一半，不因另一列高度预留空白；窄窗口自动合并为单列并禁止横向溢出。远程调试提供“读取本机设备”按钮，扫描本机 ADB 设备并优先读取 `_adb-tls-connect` mDNS 动态端口，自动配置真实 `地址:端口`，不把 `_adb-tls-pairing` 配对端口当作连接端口，不自动执行 `tcpip` 或断开当前投屏链路。
...
| Screenshots | Flutter 不提供独立工具入口；复用现有 ADB 截屏并从设备信息栏复制 PNG 到系统图片剪贴板 |
| Automation | 铃声、已配对蓝牙清理和应用商店入口；Wi‑Fi、蓝牙快捷开关位于设备信息栏 |
