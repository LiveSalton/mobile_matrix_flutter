...
- 设备卡片按真机截图的 9:20 纵向比例呈现并移除默认手机展位图；当前屏幕截图与卡片边界、圆角完全重合，名称、serial 和连接状态控件收纳在屏幕内底部完全不透明的控制浮层中，不使用毛玻璃模糊或直接压在底图上的悬浮按钮。
...
- 不实现 Airtest 执行、抖音流程、AI Agent、任务编排、iOS/WDA、批量安装应用或批量远程连接；群控执行区本期仅提供诚实的布局边界与未接入状态。
