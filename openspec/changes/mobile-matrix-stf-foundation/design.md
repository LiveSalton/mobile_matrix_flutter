...
3. 设备矩阵：使用 STF `screen.capture` 的静态当前屏幕截图作为封面，补充名称、状态文字、serial 摘要和明确选择控件，默认立即进入首屏。
   - 卡片本身按当前真机截图的 9:20 纵向比例呈现，移除额外手机展位图；截图使用 `cover` 铺满圆角卡片背景，避免两侧留白。
   - 截图层与卡片边界和圆角完全重合；设备名称、serial 和连接状态按钮全部收纳在屏幕底部、距边缘 10px 的不透明控制浮层中。浮层使用主题中性表面、12px 圆角、细描边和阴影形成明确的内部层级，不用毛玻璃、模糊或半透明遮罩；卡片外轮廓仍以主题边框覆盖绘制，不缩小截图可视区域，且信息层不遮挡顶部选择控件。卡片先占位避免异步截图造成网格跳动。
   - 没有静态截图时不渲染 STF 默认手机展位图，保留中性空白背景并由状态文字说明设备不可用。状态按钮为连接状态而不是 STF 遗留动作：可用显示“已连接 · 可控制”，使用中显示“正在控制”，忙碌显示“设备正被使用”，离线/缺席显示“连接已断开”，其余状态也使用对应中文反馈。所有状态共用 8px 圆角、至少 44px 触达高度、图标与文字居中；按钮独占信息层底部一行，状态只改变文案、图标、颜色和禁用语义，不改变布局。可用状态采用克制的主题高光底色与主题色图标，不使用整块高饱和主色填充。
   - 卡片网格按 220–260px 的可伸缩列宽排列，在小视口切换为双列紧凑布局；选择控件保留至少 44px 触达面积。
...
单设备控制页只重组现有 STF 控件，不改写 `ControlService`、minicap、minitouch、文件安装、剪贴板、远程调试或租约逻辑：

- 左侧为设备主舞台，宽度根据视口限制在 360–480px；顶部集中显示设备名称、旋转、显隐和释放操作，中部保持实时屏幕交互，底部保留 Android 菜单、Home、最近任务与返回键。
- 右侧只保留一个工作区，并在顶栏提供“群控执行 / 设备工具”双模式切换。默认进入群控执行；模式切换只替换右侧内容，不重建左侧屏幕、不释放设备，也不持久化到浏览器存储。
- 群控执行使用“选择设备 / 配置操作 / 确认执行”三步向导。第一步直接引用控制页已有 `$scope.device` 并自动显示当前设备的真实名称、serial 摘要和 STF 状态；本期不读取第二套设备集合。Airtest 未进入当前 change，因此后两步明确禁用，空状态说明执行引擎尚未接入，不渲染虚构参数、可执行按钮、模拟进度或本地计时结果。
- 设备工具使用分类侧栏与单一内容区：常用操作承载 Dashboard；记录诊断承载 Logs 与 Screenshots；设备管理承载 Automation、File Explorer、Advanced 与 Info。各项继续复用现有模板和事件，每次只挂载当前工具，删除上游重复底部标签窗格与七个同级横向标签。
- Dashboard 内容区使用最大 1180px 的单一连续表面，不重复展示左侧设备主舞台已有的名称、serial 和状态，也不再为每个 STF 小组件叠加独立外框、阴影与圆角。内部按 Quick Controls、Applications、Developer 三个任务组组织：Navigation、Clipboard 与 Shell 属于即时控制；App Upload 与五个 Apps 操作属于应用管理；Remote debug 属于低频开发信息。组间用语义分隔线和 4/8/16 间距建立层级，组内控件共用统一标题、动作、图标与输入高度；窄工作区回退单列。设备工具侧栏在紧凑视口折叠为顶部可滚动导航。所有表面、输入框、拖拽区、按钮和焦点态只使用现有 `--mm-*` 语义 token，支持液态玻璃蓝与玫瑰流光两套主题。
- 功能性 UI 图标统一使用本地打包的 Material Design Icons Round。为兼容尚未逐模板迁移的 STF 控件，控制页作用域内将旧 `fa-*` 语义类映射到对应 Material 图标，但不修改业务事件或污染设备首页。浏览器 favicon、已安装 App 图标与真机屏幕内容属于数据/内容资产，不替换为 Material 图标。
- 首次进入若 `$root.platform` 尚未初始化，控制页固定使用 STF 的 `native` 工具集合；用户仍可沿用现有快捷键切换 Web/Native，不使用设备的 `Android` 平台字符串直接过滤标签。
...
实现只使用 `--mm-*` 语义 token；共享成功、警告、错误、禁用色以及截图信息层的中性灰阶遮罩不随主题变化。采用 4/8/16/24/32/48 间距、8/12/16 圆角、150–300ms 轻量反馈、可见键盘焦点与 `prefers-reduced-motion`。设备状态同时使用文字/图标与颜色，不能只靠颜色表达。
...
设备状态变化触发的 `FatalMessage` 使用独立 `mm-device-disconnect-modal` 样式，与 Socket 断线弹窗共享 520px 响应式宽度和主题语义层级，但正文改为紧凑设备状态卡。状态卡只展示 Material `phone_android` 图标、设备名称和实时被动状态，不加载 STF 默认手机展位图；底部将“返回设备列表”作为次要操作、“重新连接”作为主操作，二者保持原有路由重载与返回列表行为。移动视口按钮纵向排列，所有操作触达高度至少 44px。
